...
export default function InfoCardLogo({
  name,
  code,
  email,
  logo = "https://api.builder.io/api/v1/image/assets/feb8ce0efd7646cb96252d2c86689552/9cb6f47f522b321eff8a9fb6fb10bd4eb689d5d5?placeholderIfAbsent=true",
  description,
  isSelected = false,
  onClick,
}: DataCenterCardProps) {
    ...
    <div className="bg-[#F8F9FA] px-[15px] py-[11px] flex items-center justify-between hover:bg-gray-100 transition-colors rounded-lg">
      <div className="flex w-full items-center gap-2.5 sm:gap-[10px]">
        ...
        <div className="flex items-center justify-start w-6 flex-shrink-0">
          <div
            className={`rounded-md border-2 flex h-6 w-6 items-center justify-center ${
              isSelected ? "border-blue-500 bg-blue-500" : "border-[#d1d5db]"
            }`}
            onClick={onClick}
        ...
        <div className="flex flex-1 items-center font-medium">
          <div className="flex w-full items-center gap-2.5 sm:gap-[10px]">
            ...
            <img
              src={logo}
              alt="avartar"
              className="w-12 h-10 flex-shrink-0 object-contain"
            />
