...
const Menu: React.FC<MenuProps> = ({ closeMenu, collapsed }) => {
    ...
    useEffect(() => {
        ...
        if (activeParent) {
            // Authoritatively set the state to ONLY the current active parent.
            setOpenSections([activeParent.label]);
        } else {
            // Optional but good practice: close sections if no link is active.
            setOpenSections([]);
        }
                                            ...
                                            <div>
                                                {item.children ? (
                                                    <>
                                                        <div
                                                            ...
                                                            className={clsx("flex items-center justify-between p-3 rounded-lg cursor-pointer hover:bg-gray-100",
                                                                isActiveParent ? "bg-gray-200" : "hover:bg-gray-100",
                                                                {'justify-center': collapsed})}
                                                        >
                                                            <div className="flex items-center gap-3">
                                                                <item.icon className="text-gray-700 min-w-[20px]"
                                                                           size={20}/>
                                                                {!collapsed && <span
                                                                    className="font-medium text-sm text-gray-800">{item.label}</span>}
                                                            </div>
                                                            {!collapsed && (
                                                                <div className="flex items-center gap-2">
                                                                    ...
                                                                    <LuChevronDown size={16}
                                                                                   className={`text-gray-500 transition-transform duration-200 ${openSections.includes(item.label) ? 'rotate-180' : ''}`}/>
                                                                </div>
                                                            )}
                                                        </div>
                                                                    ...
                                                                    .map(child => (
                                                                        <li key={child.label}>
                                                                            <Link href={child.href || '#'}
                                                                                  onClick={closeMenu}
                                                                                  className={`block p-2 rounded-md text-sm transition-colors ${pathname === child.href ? 'font-semibold text-gray-900' : 'text-gray-600 hover:text-black'}`}>
                                                                                {child.label}
                                                                            </Link>
                                                                        </li>
                                                                    ))}
                                                    ...
                                                    <Link href={item.href || '#'} onClick={closeMenu}
                                                          className={clsx("flex items-center gap-3 p-3 rounded-lg transition-colors", {
                                                              'justify-center': collapsed,
